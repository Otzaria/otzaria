...
void main() {
  test('גודל הכתב נגזר מהתקציב, כך שרוחב הטור המרונדר שווה לו', () {
  ...
  test('שיעור הפרשה נגזר משלוש פעמים "אשר" ולא ממספר תווים', () {
    ...
    expect(
      _model.setumaGapEm,
      closeTo(3 * 1.8 + 2 * _model.wordGapEm, 1e-9),
    );
    expect(
  ...
  test('רווח הפתיחה זהה לזה שהמרנדר מצייר', () {
    expect(
      _model.bigGapEm,
      closeTo(_model.lineWidthEm * kTikkunBigGapFraction, 1e-9),
    );
  });
}
