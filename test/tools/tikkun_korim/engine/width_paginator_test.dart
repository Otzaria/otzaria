...
List<double> _justifiedGaps(List<TikkunLine> lines, StamWidthModel widths) {
  ...
  for (final line in lines) {
    ...
    gaps.add((widths.lineWidthEm - used) / (items.length - 1));
  }
  return gaps;
}
...
void main() {
  ...
  late StamWidthModel charCountModel;
  ...
  setUpAll(() async {
    ...
    charCountModel = StamWidthModel.uniform(
      id: 'char-count',
      advance: widths.lineWidthEm / 36,
    );
  ...
  test('הרווח בין השורות אחיד יותר מחיתוך לפי מניין תווים', () {
    ...
    final byChars = _justifiedGaps(
      paginateAllTokens(bereshit, charCountModel),
      widths,
    );
  ...
  test('אף שורה מיושרת אינה חורגת מהתקציב', () {
    for (final line in paginateAllTokens(bereshit, widths)) {
      ...
      expect(
        ...
        lessThanOrEqualTo(widths.lineWidthEm * 1.20),
      );
    }
  });
  ...
  test('כללי הסתומה, הפתוחה והשורה החלקית נשמרים', () {
    ...
    for (final line in lines) {
      ...
      if (line.layout == LineLayout.partial) {
        ...
        expect(used, lessThan(widths.lineWidthEm * 0.65));
      }
    }
  });
  ...
  test('שיעור הפרשה תופס חלק דומה מהשורה בכל גופני הסת"ם', () async {
    ...
    for (final entry in _stamFontFiles.entries) {
      ...
      shares[entry.key] =
          measureStamWidthModel(entry.key).setumaGapEm /
          measureStamWidthModel(entry.key).lineWidthEm;
    }
