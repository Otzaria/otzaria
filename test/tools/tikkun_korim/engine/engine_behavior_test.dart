...
void main() {
  ...
  test('אף שורה מיושרת אינה נותרת ריקה למחצה', () {
    for (final line in torah.allLines) {
      ...
      final gap =
          (_widths.lineWidthEm - _lineWidthEm(line, _widths)) /
          (line.words.length - 1);
      // ignore: avoid_print
  ...
  test('אף שורה רגילה אינה חורגת מרוחב השורה', () {
    for (final line in torah.allLines) {
      ...
      expect(
        ...
        lessThanOrEqualTo(_widths.lineWidthEm),
  ...
  group('חלוקה לעמודים', () {
    final processedByMethod = <String, ProcessedTorah>{};
    ProcessedTorah processedFor(String methodId) =>
        processedByMethod.putIfAbsent(
          methodId,
          () => processTorah(
            {for (final id in TikkunData.booksOrder) id: raw[id]!},
            _widths,
    ...
    for (final methodId in ['ramah', 'ramach', 'rambamRosh']) {
      test('$methodId — כל העמודים, רצופים ומכסים את כל השורות', () {
        ...
        final processed = processedFor(methodId);
        final pages = buildPages(processed, methodId);

        expect(pages.length, layout.pages.length);
        expect(pages.first.startLineIdx, 0);
        expect(pages.last.endLineIdx, processed.allLines.length);
    ...
    for (final methodId in ['ramah', 'ramach', 'rambamRosh']) {
      test('$methodId — עמוד עם עוגן נפתח בדיוק במילת העוגן', () {
        final processed = processedFor(methodId);
        final pages = buildPages(processed, methodId);
        var anchored = 0;
        final notFirst = <int>[];
        for (var i = 1; i < pages.length; i++) {
          final tok = processed.pageStartTokenIdx[i];
          final first = pages[i].lines.first;
          if (tok == null || first.isSpecial) continue;
          anchored++;
          if (first.startTokenIdx != tok) notFirst.add(i + 1);
        }
        expect(anchored, greaterThan(210));
        expect(notFirst, isEmpty, reason: 'עמודים שהעוגן אינו בראשם');
      });
    }
