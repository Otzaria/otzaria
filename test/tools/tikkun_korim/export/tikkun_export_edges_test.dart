...
void main() {
  ...
  testWidgets('שם עליה שיש לו חלופה נאסף — בלי Overlay אין Tooltip', (
    tester,
  ) async {
    ...
    await tester.runAsync(() async {
      pages = await exporter.layoutPages([
        [
          textLine(['אחת'], chapter: 1, verse: 1, aliyaName: 'שני', aliyaIdx: 1)
            ..aliyaAlternative = 'ד,יט–ד,כו',
        ],
      ]);
    });

    expect(_withText(pages.single, 'שני'), hasLength(1));
  });
