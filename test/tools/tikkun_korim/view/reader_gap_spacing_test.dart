...
void main() {
  ...
  setUpAll(() {
    _metrics = TikkunRenderMetrics.forWidth(
      1200,
      _settings,
  ...
  test('שורת פתוחה שמילאה את הטור מיושרת כרגילה ואינה גולשת', () {
    final row =
        buildTikkunWordsRow(
              words: _words.map(word).toList(),
              layout: LineLayout.petucha,
              metrics: _metrics,
              contentWidth: _used + (_words.length - 1) * _minGap - 1,
              isStam: true,
              maskDivineName: false,
              averageGap: _minGap * 8,
            )
  ...
  test('הממוצע נגזר מהשורות המיושרות בלבד', () {
    ...
    // הרווח הממוצע נמדד אחרי סגירת השורה — המתיחה נכללת ברוחב המילים.
