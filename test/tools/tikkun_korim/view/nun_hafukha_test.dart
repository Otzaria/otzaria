/// נו"ן מנוזרת: בגופן שיש בו גליף לתו עצמו הוא מרונדר כמות שהוא, ובגופן
/// שאין בו שני הטורים מהפכים נו"ן רגילה. עם TIKKUN_VISUAL=1 נשמר PNG.
library;
...
/// גופנים שאין בהם גליף לנו"ן מנוזרת — מסלול ההיפוך.
const TikkunSettings _mirroringSettings = TikkunSettings(
  stamFont: 'Ashkenazi',
  nikudFont: 'System:KeterYG',
);

Future<void> _pumpWords(
  WidgetTester tester,
  String text, {
...
void main() {
  ...
  testWidgets('בגופן שיש בו גליף — התו עצמו מרונדר בלי היפוך', (tester) async {
    await _pumpWords(tester, kNunHafukha);

    for (final column in [find.byType(StamWord), find.byType(NikudWord)]) {
      expect(
        tester.widgetList(
          find.descendant(of: column, matching: find.byType(Transform)),
        ),
        isEmpty,
      );
      expect(_textsUnder(tester, column), everyElement(equals(kNunHafukha)));
    }
  });

  testWidgets('בגופן שאין בו גליף — שני הטורים מהפכים נו"ן רגילה', (
    tester,
  ) async {
    await _pumpWords(tester, kNunHafukha, settings: _mirroringSettings);
    ...
    for (final column in [find.byType(StamWord), find.byType(NikudWord)]) {
      ...
      expect(tester.widgetList(mirrored), hasLength(1));
  ...
  testWidgets('הרוחב הנמדד שווה לרוחב המרונדר בשני הטורים', (tester) async {
    const text = 'וּ׆יְהִי';
    await _pumpWords(tester, text, settings: _mirroringSettings);

    const settings = _mirroringSettings;
    final metrics = TikkunRenderMetrics.forWidth(
      1200,
      settings,
      lineWidthEm: kTestLineWidthEm,
  ...
  test('בגופן ללא גליף, רוחב הנו"ן המנוזרת הוא רוחב הנו"ן הרגילה', () {
    final model = measureStamWidthModel(_mirroringSettings.stamFontFamily);
