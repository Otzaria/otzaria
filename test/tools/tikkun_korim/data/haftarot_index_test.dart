...
void main() {
  ...
  group('ערכים כפולים או שאינם נקראים הוסרו', () {
    test('וילך לבדה חלה תמיד בשבת שובה — אין לה ערך משלה', () {
      final ids = haftarot.map((h) => h.id).toSet();
      expect(ids, isNot(contains('p:Vayeilech')));
      expect(ids, contains('p:Nitzavim-Vayeilech'));
    });

    test('ערכים זהים לערך אחר או שאינם נקראים', () {
      final ids = haftarot.map((h) => h.id).toSet();
      for (final id in [
        'h:Kedoshim following Special Shabbat',
        'h:Pinchas occurring after 17 Tammuz',
        'h:Masei on Shabbat Rosh Chodesh',
        'h:Masei on Shabbat Rosh Chodesh (Israel)',
        'p:Matot-Masei',
      ]) {
    ...
    test('חנוכה: שבת חנוכה ושבת זאת חנוכה בלבד', () {
      final chanukah = haftarot.where((h) => h.id.contains('Chanukah'));
      expect(chanukah.map((h) => h.id), [
        'h:Shabbat Chanukah',
        'h:Shabbat Zot Chanukah',
      ]);
      expect(_byId('h:Shabbat Chanukah').ashkenaz.single.book, 'זכריה');
      expect(_byId('h:Shabbat Zot Chanukah').ashkenaz.single.book, 'מלכים א');
    });

    test('שלוש השבתות: פנחס בי"ז בתמוז, "דברי ירמיהו", "שמעו"', () {
      expect(_byId('p:Pinchas').name, 'פנחס - כשחל בי"ז בתמוז');
      expect(_byId('p:Pinchas').ashkenaz.single.book, 'מלכים א');
      expect(_byId('p:Matot').name, 'פנחס או מטות');
      expect(_byId('p:Matot').ashkenaz.single.fromCh, 1);
      expect(_byId('p:Masei').name, 'מטות-מסעי או מסעי');
      expect(_byId('p:Masei').ashkenaz.first.fromCh, 2);
    });
    ...
    test('שבת שובה — ערך יחיד; אשכנז מוסיפים "תקעו"', () {
      final shuva = haftarot.where((h) => h.id.contains('Shabbat Shuva'));
      expect(shuva.map((h) => h.id), ['h:Shabbat Shuva']);
      expect(shuva.single.forNusach('ashkenaz').map((r) => r.book), [
        'הושע',
        'מיכה',
        'יואל',
      ]);
      expect(shuva.single.forNusach('sephard'), const [
