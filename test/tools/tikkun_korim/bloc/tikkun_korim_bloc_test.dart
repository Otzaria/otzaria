...
void main() {
  ...
  group('מיקום מדויק וסימניות', () {
    const lastPosition = TikkunSettings(startupMode: 'lastPosition');

    Future<void> settle() =>
        Future<void>.delayed(const Duration(milliseconds: 20));

    test('במצב "מיקום אחרון" חוזרים לשורה שנקראה ולא לראש הפרשה', () async {
      store = FakeTikkunSettingsStore(
        settings: lastPosition,
        navState: const TikkunNavState(methodId: 'single_page'),
      );
      final first = buildBloc()..add(const TikkunStarted());
      await settle();
      first.add(const TikkunVisibleLineChanged(5));
      await settle();
      await first.close();
      expect(store.navState.verseKey, isNotNull);

      final second = buildBloc()..add(const TikkunStarted());
      await settle();
      expect(second.state.scrollToLine, 5);
      await second.close();
    });

    test('במצב "מיקום אחרון" בעמודים חוזרים לעמוד שנקרא', () async {
      store = FakeTikkunSettingsStore(settings: lastPosition);
      final first = buildBloc()..add(const TikkunStarted());
      await settle();
      first.add(const TikkunColumnSelected(4));
      await settle();
      expect(first.state.nav.currentColumnIndex, 4);
      await first.close();

      final second = buildBloc()..add(const TikkunStarted());
      await settle();
      expect(second.state.nav.currentColumnIndex, 4);
      await second.close();
    });

    test('סימניה נשמרת ומחזירה לשורה שבה נוספה', () async {
      store = FakeTikkunSettingsStore(
        settings: lastPosition,
        navState: const TikkunNavState(methodId: 'single_page'),
      );
      final bloc = buildBloc()..add(const TikkunStarted());
      await settle();
      bloc.add(const TikkunVisibleLineChanged(4));
      await settle();
      bloc.add(const TikkunBookmarkAdded());
      await settle();
      expect(bloc.state.bookmarks, hasLength(1));
      expect(store.bookmarks, hasLength(1));
      expect(bloc.state.bookmarks.single.title, contains('בראשית'));

      bloc.add(const TikkunVisibleLineChanged(0));
      await settle();
      bloc.add(TikkunBookmarkOpened(bloc.state.bookmarks.single));
      await settle();
      expect(bloc.state.scrollToLine, 4);

      bloc.add(TikkunBookmarkRemoved(bloc.state.bookmarks.single.id));
      await settle();
      expect(store.bookmarks, isEmpty);
      await bloc.close();
    });

    test('סימניה עוברת JSON בלי אובדן', () {
      const bookmark = TikkunBookmark(
        id: '1',
        title: 'שמות ג, א',
        nav: TikkunNavState(
          bookId: 'shemot',
      ...
      expect(TikkunBookmark.fromJson(bookmark.toJson()), bookmark);
      expect(TikkunBookmark.fromJson('פגום'), isNull);
    });
  });
  ...
  group('סנכרון לפי גלילה', () {
    ...
    blocTest<TikkunKorimBloc, TikkunKorimState>(
      'מעבר לעמוד שאין בו פתיחת פרשה מעדכן את הפרשה מעמודים קודמים',
      build: () {
        store = FakeTikkunSettingsStore(
          settings: const TikkunSettings(startupMode: 'lastPosition'),
          navState: const TikkunNavState(parashaName: 'בראשית'),
        );
        return buildBloc();
      },
      act: (bloc) async {
        bloc.add(const TikkunStarted());
        await Future<void>.delayed(const Duration(milliseconds: 10));
        bloc.add(const TikkunColumnSelected(3));
      },
      wait: const Duration(milliseconds: 30),
      verify: (bloc) {
        expect(bloc.state.nav.currentColumnIndex, 3);
        expect(bloc.state.nav.parashaName, 'נח');
      },
    );

    blocTest<TikkunKorimBloc, TikkunKorimState>(
      'חזרה לעמוד של פרשה קודמת מחזירה את הבורר אליה',
      build: () {
        store = FakeTikkunSettingsStore(
          settings: const TikkunSettings(startupMode: 'lastPosition'),
          navState: const TikkunNavState(parashaName: 'בראשית'),
      ...
      act: (bloc) async {
        ...
        bloc.add(const TikkunColumnSelected(3));
        await Future<void>.delayed(const Duration(milliseconds: 10));
        bloc.add(const TikkunColumnSelected(0));
      },
      wait: const Duration(milliseconds: 30),
      verify: (bloc) => expect(bloc.state.nav.parashaName, 'בראשית'),
    );
  });
}
