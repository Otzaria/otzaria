/// דיאלוג בחירת התוכן להדפסה של "תיקון קוראים" — לפני מסך ההדפסה.
library;
...
/// רוחב תוכן הדיאלוג — מספיק לחמש אפשרויות היקף בשורה אחת.
...
Future<TikkunExportOptions?> showTikkunExportDialog({
  ...
  final parasha = parashaName == null ? null : parashaRange;
  final defaultScope = initial.scope == TikkunExportScope.currentColumn;
  var options = initial.copyWith(
    fromColumn: current,
    toColumn: current,
    parashaFromColumn: parasha?.from,
    parashaToColumn: parasha?.to,
    // בטור יחיד "העמוד הנוכחי" ו"הכל" זהים — מוצג רק "הכל".
    scope: parasha != null && defaultScope
        ? TikkunExportScope.parasha
        : total == 1 && defaultScope
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            if (_options.scope == TikkunExportScope.parasha)
              Padding(
                padding: const EdgeInsets.only(top: 8),
                child: Text(
                  _options.mode == TikkunExportMode.flow
                      ? 'פרשת ${widget.parashaName}'
                      : 'פרשת ${widget.parashaName}, עמודים '
                            '${_options.parashaFromColumn + 1}–'
                            '${_options.parashaToColumn + 1}',
