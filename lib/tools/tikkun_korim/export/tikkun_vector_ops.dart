...
class TikkunTextOp {
  ...
  /// מתיחה אופקית של הגליפים סביב [left] — רצועת הגג של אות נמתחת.
  final double horizontalScale;

  /// חיתוך לרצועה מתוך אות נמתחת, בקואורדינטות העמוד.
  final Rect? clip;

  /// עותק נוסף של אות שכבר נכתבה — מצויר, אך לא ייכלל בהעתקת הטקסט.
  final bool artifact;
  ...
  /// קצוות החלק הנראה בפועל, אחרי [clip].
  double get inkLeft => clip == null || clip!.left < left ? left : clip!.left;
  double get inkRight =>
      clip == null || clip!.right > right ? right : clip!.right;
...
TikkunVectorPage collectTikkunVectorPage(
  ...
  void visit(RenderObject node) {
    ...
    final clipper = node is RenderClipRect ? node.clipper : null;
    if (clipper is TikkunLetterSliceClipper) {
      final saved = clip;
      clip = MatrixUtils.transformRect(
        node.getTransformTo(root),
        clipper.getClip((node as RenderBox).size),
      );
    ...
    if (node is RenderParagraph) {
      _collectParagraph(node, root, out, clip);
      return;
    }
  ...
  final keyed =
      [
        for (var index = 0; index < ops.length; index++)
          (
            slot: columns.slotOf(
              (ops[index].inkLeft + ops[index].inkRight) / 2,
            ),
        ...
        final left = a.op.inkLeft.compareTo(b.op.inkLeft);
        ...
        for (final box in boxes) {
          ...
          out.add(
            TikkunTextOp(
              ...
              horizontalScale: transform.storage[0].abs(),
              clip: clip,
              // רק הרצועה שמתחילה בקצה האות נושאת את הטקסט.
              artifact: clip != null && clip.left > globalRect.left + 0.01,
            ),
          );
        }
      }
