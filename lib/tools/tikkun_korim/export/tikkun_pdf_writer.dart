...
/// מחזיר את השליטה ללולאת האירועים. `await` על Future שהושלם רץ כ-microtask
/// ואינו מאפשר פריים — ובווינדוס גם לא טיפול בהודעות החלון.
...
class TikkunVectorPdfWriter {
  ...
  double get _usableWidth => pageFormat.width - 2 * kTikkunExportMarginPt;

  double get _usableHeight => pageFormat.height - 2 * kTikkunExportMarginPt;
  ...
  /// כותב את [pages] ומחזיר את בייטי המסמך. עם [unifyScale] עמודים שווי רוחב
  /// מוקטנים לקנה המידה של הצפוף שבהם; דף שירה רחב מוקטן בנפרד.
  Future<Uint8List> write(
    ...
    if (unifyScale) {
      ...
      for (var i = 0; i < pages.length; i++) {
        shared.update(
          pages[i].width,
          (s) => math.min(s, scales[i]),
          ifAbsent: () => scales[i],
        );
      }
      for (var i = 0; i < pages.length; i++) {
        scales[i] = shared[pages[i].width]!;
      }
    }
    try {
      for (var i = 0; i < pages.length; i++) {
  ...
  Future<void> _writePage(TikkunVectorPage page, double scale) async {
    ...
    final originX = (pageFormat.width - page.width * scale) / 2;
    final top = pageFormat.height - kTikkunExportMarginPt;
    ...
    for (final op in page.texts) {
      ...
      if (clip != null) {
        g.drawRect(
          originX + clip.left * scale,
          top - clip.bottom * scale,
          clip.width * scale,
          clip.height * scale,
        );
        g.clipPath();
      }
      if (op.artifact) g.markContentBegin(const PdfName('/Artifact'));
      ...
      final stretch = op.horizontalScale;
      if (stretch != 1) {
        g.setTransform(
          Matrix4(
            stretch,
      ...
      if (op.artifact) g.markContentEnd();
      g.restoreContext();
    }
  }
  ...
  void _writeWordSpace(
    ...
    final leftBox = op.inkLeft < previous.inkLeft ? op : previous;
    final rightBox = op.inkLeft < previous.inkLeft ? previous : op;
    if (rightBox.inkLeft - leftBox.inkRight < op.fontSize * _kMinWordGapEm) {
      return;
    }
    ...
    font.pdfFont.drawShapedRun(
      g,
      space,
      x: originX + leftBox.inkRight * scale,
