...
List<double> tikkunStretchLetterWidths(String text, TextStyle style) {
  ...
  return [
    for (final part in splitTikkunNun(text, style.fontFamily))
...
/// שהמרנדר הופך. בגופן שיש בו גליף לנו"ן המנוזרת אין מה לפרק.
/// מקור יחיד למרנדר ולמדידת הרוחב, כדי ששניהם ימדדו את אותם קטעים.
List<String> splitTikkunNun(String text, [String? family]) {
  if (tikkunFamilyHasNunHafukha(family)) return [text];
...
Widget tikkunWordText(
  ...
  final parts = splitTikkunNun(text, style.fontFamily);
  final stretch =
      letterExtras.any((e) => e > 0) && tikkunStretchableCount(text) > 0;
  // קטע יחיד שאינו המילה עצמה הוא נו"ן מנוזרת בודדת, והמרנדר חייב להפוך אותה.
  if (!stretch && parts.length == 1 && parts.first == text) {
    return condensed(chunk(text));
  }
  var letterIdx = 0;
  double nextExtra() =>
