...
enum TikkunTokenType {
  ...
  /// תחילת עמוד כפויה — מוזרק לפני מילת העוגן של עמוד בשיטה.
  pageBreak,
}
...
class LayoutWord extends Equatable {
  ...
  /// רווח סתומה: חלקו משיעור הפרשה — פחות מ-1 בהשלמה שבראש שיטה (רמב"ם ח,ב).
  ...
  @override
  List<Object?> get props => [stam, nikud, verseNum, tokenIdx, gapFraction];
}
...
class TikkunLine {
  ...
  /// רוחב הדף שהשורה בו ביחס לתקציב הבסיסי — מעל 1 בדפי השירה.
  double widthFactor = 1;

  /// הרווח שבין תאי שורה מיושרת, בשיעור מרווח פרשה — מצטמצם בשירה צפופה.
  double cellGapFactor = 1;

  /// התקציב שהשורה נחתכה בו, באותן יחידות; המרנדר סוגר את הפער עד [widthFactor].
  double budgetFactor = 1;

  /// חלק רוחב השורה שנותר מעבר לתקציב — בשורה מיושרת בלבד; שלילי בעמוד צפוף
  /// שהתצוגה מכווצת.
  double get slackFraction => switch (layout) {
    LineLayout.regular ||
    LineLayout.setuma ||
    LineLayout.setumaStart => 1 - budgetFactor / widthFactor,
    _ => 0,
  };
...
class PageDefinition extends Equatable {
  ...
  /// המילה הראשונה והאחרונה בעמוד בלי ניקוד, כפי שהן בטבלת החלוקה.
  final String? firstWord;
  final String? lastWord;

  const PageDefinition({required this.num, this.firstWord, this.lastWord});

  @override
  List<Object?> get props => [num, firstWord, lastWord];
}

/// מילה בתורה: חומש (1–5 לפי סדר החומשים), פרק, פסוק ומספרה בפסוק.
typedef TikkunWordRef = ({int book, int chapter, int verse, int word});
...
class ProcessedTorah {
  ...
  /// אסימון המילה שפותחת כל עמוד בשיטה שלפיה עובד הטקסט; `null` = אין עוגן.
  final List<int?> pageStartTokenIdx;

  const ProcessedTorah({
    required this.tokens,
    required this.allLines,
    required this.bookStartTokenIdx,
    this.pageStartTokenIdx = const [],
  });
}
