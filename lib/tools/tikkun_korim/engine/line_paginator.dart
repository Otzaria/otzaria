...
/// תקרת השיטות שנחתכות מחדש יחד — כעמוד, כדי שהחיתוך יישאר זול בכיול העמודים.
const int _kMaxRebalanceLines = 42;

/// האם אחרי [i] באה ממילא שיטה פנויה: מעבר חומש, שירה שנפתחת בה, או סוף הטקסט.
bool _blankLineFollows(List<TikkunToken> tokens, int i) {
  for (var j = i + 1; j < tokens.length; j++) {
    final tok = tokens[j];
    if (tok.type == TikkunTokenType.chapterBreak ||
        tok.type == TikkunTokenType.verseBreak) {
      continue;
    }
    return tok.type == TikkunTokenType.bookBreak ||
        (tok.type == TikkunTokenType.specialStart &&
            (tok.section?.blankLineBefore ?? false));
  }
  return true;
}
...
List<TikkunLine> paginateAllTokens(
  ...
  // אחרי שתיבות ירדו, השיטות הרגילות שמעליהן עד הגבול הקודם נחתכות מחדש יחד,
  // כך שהעודף מתחלק על כל הקטע ולא נשאר בשיטה האחרונה. מניין השיטות נשמר.
  void rebalanceTrailingRun() {
    var first = lines.length;
    while (first > 0 && lines.length - first < _kMaxRebalanceLines) {
      final line = lines[first - 1];
      final meta = lineMeta[first - 1];
      if (line.layout != LineLayout.regular ||
          meta == null ||
          meta.isEmpty ||
          meta.length != line.words.length ||
          line.words.any((w) => w.isGap || w.isBigGap)) {
        break;
      }
      if (first < lines.length &&
          _breakBetween(
            tokens,
            meta.last.tokenIdx,
            lineMeta[first]!.first.tokenIdx,
          )) {
        break;
      }
      first--;
    }
    final count = lines.length - first;
    if (count < 2) return;

    final words = [
      for (var k = first; k < lines.length; k++) ...lines[k].words,
    ];
    final metas = [for (var k = first; k < lines.length; k++) ...lineMeta[k]!];
    final itemWidths = [for (final w in words) widths.itemWidthEm(w)];
    final budget =
        segmentWidths[metas.first.tokenIdx.clamp(
          0,
          segmentWidths.length - 1,
        )];
    final n = words.length;

    // cost[j] אחרי k סבבים: העלות הזולה ל-j התיבות הראשונות ב-k שיטות.
    var cost = List<double>.filled(n + 1, double.infinity)..[0] = 0;
    final start = List.generate(count, (_) => List<int>.filled(n + 1, -1));
    for (var k = 0; k < count; k++) {
      final next = List<double>.filled(n + 1, double.infinity);
      for (var j = 1; j <= n; j++) {
        var used = 0.0;
        for (var i = j - 1; i >= 0; i--) {
          used += itemWidths[i];
          ...
          final slots = j - i - 1;
          final extra = slots == 0 ? budget - used : (budget - used) / slots;
          if (cost[i] + extra * extra < next[j]) {
            next[j] = cost[i] + extra * extra;
            start[k][j] = i;
          }
        }
      }
    ...
    if (cost[n] == double.infinity) return;

    var end = n;
    for (var k = count - 1; k >= 0; k--) {
      final from = start[k][end];
      lines[first + k].words = words.sublist(from, end);
      lineMeta[first + k] = metas.sublist(from, end);
      recomputeMarkers(first + k);
      end = from;
    }
  }
  ...
  // התיבות האחרונות פותחות שיטה חדשה, והשיטה שהתקצרה לוקחת מילים מקודמותיה.
  void moveLastWordsToNewLine(int count) {
    final from = currentLine.length - count;
    final moved = currentLine.sublist(from);
    final movedMeta = currentMeta.sublist(from);
    currentLine.removeRange(from, currentLine.length);
    currentMeta.removeRange(from, currentMeta.length);
    flushLine();
    rebalanceTrailingRun();
    currentLineStartTokenIdx = movedMeta.first.tokenIdx;
    for (final meta in movedMeta) {
      currentLineFirstChapter ??= meta.chapterNum;
      currentLineFirstVerse ??= meta.verseNum;
    }
    currentLine.addAll(moved);
    currentMeta.addAll(movedMeta);
    for (final word in moved) {
      lineWidth += widths.itemWidthEm(word);
    }
  }
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (tok.type == TikkunTokenType.petucha) {
      ...
      // בלי שיעור פרשה בסוף השיטה הפתוחה נראית כמלאה; יורדות שתי תיבות,
      // כדי שבשיטה החדשה לא תעמוד תיבה יתומה.
      bool isWordAt(int k) =>
          k >= 0 && !currentLine[k].isGap && !currentLine[k].isBigGap;
      final n = currentLine.length;
      if (!rambamParashaForms &&
          isWordAt(n - 1) &&
          isWordAt(n - 2) &&
          lineWidthEm - lineWidth < widths.setumaGapEm &&
          !_blankLineFollows(tokens, i)) {
        moveLastWordsToNewLine(isWordAt(n - 3) ? 2 : 1);
      }
      if (currentLine.isNotEmpty) {
        lineLayout = LineLayout.petucha;
        flushLine();
