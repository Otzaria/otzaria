  ...
  Widget buildExpansiomTile(
    ...
    final backgroundColor = () {
      if (isPermanentHighlight) {
        return (theme.brightness == Brightness.light ? Colors.yellow : Colors.orange.shade900).withValues(alpha: 0.45);
      }
      if (isHighlighted) {
                  ...
                  builder: (context, constraints) {
                    return BlocBuilder<SettingsBloc, SettingsState>(
                      builder: (context, settingsState) {
                        ...
                        // הדגשת טקסט חיפוש — highlightText מוגבל לשורה הספציפית בלבד
                        final textWidget = SmartTextWidget(
                          ...
                          settings: RenderSettings(
                            ...
                            searchOptions: state.isHighlightYellowBackground(index) ? const {} : state.searchOptions,
                            alternativeWords: state.alternativeWords,
                            spacingValues: state.spacingValues,
                            isFuzzySearch: state.isHighlightYellowBackground(index) ? false : state.searchMode == SearchMode.fuzzy,
                            searchMode: state.isHighlightYellowBackground(index) ? SearchMode.exact : state.searchMode,
                            searchDistance: state.isHighlightYellowBackground(index) ? 0 : state.searchDistance,
                            fontSize: widget.textSize,
                            fontFamily: settingsState.fontFamily,
                            lineHeight: settingsState.lineHeight,
                          ),
