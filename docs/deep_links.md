...
### `otzaria reports` — העברת דיווחים שמורים ממחשב לא מחובר (CLI)

פקודת headless נוספת, שאינה כתובת URI: תוכנה חיצונית (למשל כלי עדכון מכונן נייד) מעבירה בעזרתה את דיווחי הטעויות בספרים שבתור ממחשב לא מחובר למחשב מחובר. היא נרשמת ב-`IsCliInvocation` בדיוק כמו `info`, ולכן עובדת גם כשאוצריא פתוחה. תיקיית המסדים נגזרת מההגדרות דרך `SettingsSnapshot`, כולל המצב הנייד. דיווחי תוספים ודיווחים על התוכנה אינם בכלל זה.

```text
otzaria.exe reports pending --out=pending.json   # ספירת התור, בלי שינוי
otzaria.exe reports handoff --out=E:\reports     # קובץ לכל דיווח
otzaria.exe reports --help
```

| פעולה | פלט |
|--------|--------|
| `pending` | `{"format":"otzaria-reports-pending","version":1,"pending":N}` לקובץ `--out` (או ל-stdout). N הוא הדיווחים בתור שאפשר לשלוח, ידניים ואוטומטיים |
| `handoff` | `<dir>/<report_id>.json` לכל דיווח: `{"format":"otzaria-report","version":1,"report_id","endpoint","book_title","created_at","body"}`, ו-`body` הוא בדיוק גוף ה-POST (`toApiPayload`) |

ב-`handoff` כל קובץ נכתב ל-`.tmp` ומשנה את שמו, ורק אחרי כן הדיווח עובר מהתור להיסטוריית הנשלחים, כמו "סמן כנשלח" (רשומה קודמת באותו מזהה מוחלפת). תווים שאינם `A-Z a-z 0-9 _ -` בשם הקובץ מוחלפים ב-`_`. אם השם כבר תפוס בקובץ של דיווח אחר, מתווספת לו סיומת יציבה מה-hash של המזהה; קובץ של אותו דיווח נדרס, ולכן הרצה חוזרת בטוחה.

- **דיווח שלא נכתב** נשאר בתור.
- **דיווח שאוצריא הפתוחה ערכה או שלחה בזמן ההעברה** — השינוי שלה גובר: הקובץ כבר נכתב, אבל הדיווח אינו מסומן ונספר בסיכום בנפרד. שליחה כפולה מסוננת בשרת לפי `content_digest`.
- **דיווח פסול** (אין לו digest קנוני, למשל surrogate בודד) היה נדחה בשרת. הוא אינו נספר ב-`pending`, ב-`handoff` הוא מדולג עם שורה ב-stderr שמציינת את מזההו, נשאר בתור לעריכה, ואינו מכשיל את קוד היציאה.
- **לשונית הגדרות פתוחה** אינה מתרעננת מעצמה: היא עלולה להציג את הדיווחים שהועברו כממתינים עד שתיפתח מחדש.

| קוד יציאה | משמעות |
|--------|--------|
| `0` | הצלחה, גם כשהתור ריק, כשאוצריא לא הופעלה במחשב מעולם (אין קובץ הגדרות) או כשאין עדיין מסד (`pending` מחזיר 0, ושום קובץ או תיקייה אינם נוצרים) |
| `1` | כשל, והסיבה ב-stderr: קובץ ההגדרות קיים אך לא נקרא (מיקום התור אינו ידוע), או ב-`handoff` — כתיבת דיווח נכשלה. דיווחים שנכתבו כבר סומנו |
| `2` | התור עדיין בפורמט הישן (`<dataRoot>/error_reports_queue.hive`): הוא מועבר למסד רק כשאוצריא עולה. יש לפתוח אותה פעם אחת ולנסות שוב. דבר אינו משתנה |
| `64` | שגיאת שימוש (פעולה חסרה או לא מוכרת, `handoff` בלי `--out`) |

**זיהוי יכולת בלי להריץ את התוכנה:** גרסה ישנה הייתה מעלה ממשק מלא על פקודה לא מוכרת. לכן קורא חיצוני בודק תחילה את `cli_capabilities.json` (`{"version":1,"capabilities":["reports-pending-v1","reports-handoff-v1"]}`), ומריץ רק כשהיכולת מופיעה בו. מיקומו:

- **Windows:** `<תיקיית otzaria.exe>\data\flutter_assets\assets\cli_capabilities.json`
- **Linux:** `<תיקיית קובץ ההרצה otzaria>/data/flutter_assets/assets/cli_capabilities.json`
