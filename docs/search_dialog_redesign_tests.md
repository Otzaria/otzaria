...
- קיים `IconButton` עם tooltip `ברירות מחדל לחיפוש רגיל`, והוא באותו `dy`
  של הכותרת `הרחבת המילים` (ולא בשורה נפרדת מתחת לשבבים).
...
### 1.19 מצב מקורב אינו מציג את שורת ההרחבה

**למה:** `_buildModeContent` מוסיף את שורת אפשרויות המילה רק ב-exact ו-advanced.

**מה לבדוק:** במצב מקורב — `find.text('הרחבת המילים')` הוא `findsNothing`,
...
### 9.1 תפריט ברירות המחדל בשורת "הרחבת המילים"
